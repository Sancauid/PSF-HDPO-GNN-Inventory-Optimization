...
class DifferentiableSimulator:
    ...
    def reset(
        ...
        if self.lead_time_stores > 0:
            self.pipeline_stores = torch.zeros(
                B, S, self.lead_time_stores, device=self.device, dtype=self.dtype
            )
        else:
            self.pipeline_stores = None
        if self.lead_time_warehouses > 0:
            self.pipeline_warehouses = torch.zeros(
                B, W, self.lead_time_warehouses, device=self.device, dtype=self.dtype
            )
        else:
        ...
        self.holding_store_rate = (
            hs
            if isinstance(hs, torch.Tensor)
            else torch.tensor(hs, device=self.device, dtype=self.dtype)
        ).reshape(-1)
        self.underage_store_rate = (
            us
            if isinstance(us, torch.Tensor)
            else torch.tensor(us, device=self.device, dtype=self.dtype)
        ).reshape(-1)
        self.holding_warehouse_rate = (
            hw
            if isinstance(hw, torch.Tensor)
            else torch.tensor(hw, device=self.device, dtype=self.dtype)
        ).reshape(-1)
        ...
        if self.holding_warehouse_rate.numel() == 1:
            self.holding_warehouse_rate = self.holding_warehouse_rate.expand(
                self.num_warehouses
            )
        ...
        return {
            ...
            "pipeline_stores": (
                self.pipeline_stores
                if self.pipeline_stores is not None
                else torch.zeros(0, device=self.device, dtype=self.dtype)
            ),
            "pipeline_warehouses": (
                self.pipeline_warehouses
                if self.pipeline_warehouses is not None
                else torch.zeros(0, device=self.device, dtype=self.dtype)
            ),
    ...
    def step(
        ...
        """
        assert (
            self.inventory_stores is not None and self.inventory_warehouses is not None
        )
        assert (
            self.demands is not None
            and self.holding_store_rate is not None
            and self.underage_store_rate is not None
            and self.holding_warehouse_rate is not None
        )
        ...
        a_w = (
            actions["warehouses"].to(device=self.device, dtype=self.dtype).reshape(B, W)
        )
        ...
        if self.lead_time_warehouses > 0 and self.pipeline_warehouses is not None:
            ...
            self.pipeline_warehouses = torch.cat(
                [self.pipeline_warehouses[:, :, 1:], a_w.unsqueeze(-1)], dim=2
            )
        else:
            arrivals_w = a_w
        ...
        feasible_factor = torch.where(
            total_ship_req > 0,
            torch.clamp(total_wh_inv / total_ship_req, max=1.0),
            torch.ones_like(total_ship_req),
        )
        ...
        if self.lead_time_stores > 0 and self.pipeline_stores is not None:
            ...
            self.pipeline_stores = torch.cat(
                [self.pipeline_stores[:, :, 1:], a_s_feasible.unsqueeze(-1)], dim=2
            )
        else:
        ...
        underage_store_cost = (underage * self.underage_store_rate.view(1, S)).sum(
            dim=1
        )
        holding_warehouse_cost = (
            self.inventory_warehouses * self.holding_warehouse_rate.view(1, W)
        ).sum(dim=1)
